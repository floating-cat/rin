...
in
{
  ...
  home.file = mkIf isLinux {
    ...
    "${config.xdg.configHome}/autostart/ssh-add.desktop".text = ''
      [Desktop Entry]
      Exec=ssh-add -q .ssh/id_ed25519_youri .ssh/id_ed25519_rin
      Name=ssh-add
      Type=Application
    '';
    ...
    "${config.xdg.configHome}/vim/helix.vim".source = builtins.fetchurl {
      ...
      sha256 = "00ammdghj17j835x1nlzn5c9k5wjyp3bbh3ds4yhfxmhdbhincgg";
    };
  };
  ...
  programs = {
    ...
    git = {
      enable = true;
      package = pkgs.emptyDirectory;
      userEmail = "monsoon.cl@gmail.com";
      userName = "floating-cat";
    };
  ...
  sops = {
    ...
    secrets = builtins.mapAttrs (_: value: value // { mode = "0600"; }) {
      "ssh_keys/id_ed25519_youri" = {
        path = "${config.home.homeDirectory}/.ssh/id_ed25519_youri";
      };
